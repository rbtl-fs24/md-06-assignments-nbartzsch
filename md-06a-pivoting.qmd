...
#the tidyverse is a package containing various other packages such as dplyr ggplot2 and others that make data manipulation easier. Also with packages like ggplot data visualization is very accessible. 
```
...
ggplot(data = waste_data_long_mean,
  ...
  labs(title = "Waste Composition",
       subtitle = "Mean percentages of nine categories dispalyed by income categories",
       y = "Waste Category",
       x = "Mean (Percent)",
...
#i am not sure how to add the names of the waste categories to the plot. 
```
